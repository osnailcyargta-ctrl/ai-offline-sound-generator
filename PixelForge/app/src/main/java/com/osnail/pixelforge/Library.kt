...
data class Sample(val file: String, val label: String)
...
class Library(private val ctx: Context) {
    ...
    val imagesDir: File = File(ctx.filesDir, "images").apply { mkdirs() }
    val libFile: File = File(ctx.filesDir, "library.json")
    val matcherFile: File = File(ctx.filesDir, "matcher.json")
        ...
        try {
            ...
            for (i in 0 until arr.length()) {
                ...
                for (j in 0 until sa.length()) {
                    ...
                    cat.samples.add(Sample(s.getString("file"), s.optString("label", "")))
                }
                categories.add(cat)
            }
        } catch (e: Exception) {
    ...
    fun save() {
        ...
        for (c in categories) {
            ...
            val sa = JSONArray()
            for (s in c.samples) sa.put(JSONObject().put("file", s.file).put("label", s.label))
            o.put("samples", sa)
            arr.put(o)
        }
        root.put("version", 1)
    ...
    fun deleteCategory(name: String) {
        ...
        for (s in c.samples) File(imagesDir, s.file).delete()
        categories.remove(c)
        save()
    }

    fun addSample(cat: Category, img: PixelImage, label: String = "drawing"): Sample {
        val fname = "p_${System.currentTimeMillis()}_${(0..9999).random()}.pix"
        img.write(File(imagesDir, fname))
        val s = Sample(fname, label)
        cat.samples.add(s)
        save()
        return s
    }

    fun deleteSample(cat: Category, s: Sample) {
        File(imagesDir, s.file).delete()
        cat.samples.remove(s)
        save()
    }

    fun loadSample(s: Sample): PixelImage {
        val f = File(imagesDir, s.file)
        return if (f.exists()) PixelImage.read(f) else PixelImage()
    }

    fun totalSamples(): Int = categories.sumOf { it.samples.size }

    fun storageBytes(): Long = imagesDir.listFiles()?.sumOf { it.length() } ?: 0L

    fun saveTrainMeta(categories: List<String>, samplesUsed: Int, epochs: Int, finalLoss: Double) {
        val o = JSONObject()
